...
interface IContext {
  ...
  ethereumRpc: {
    ...
    testEthCall: TRpcRequestCallback;
    testRequestAccounts: TRpcRequestCallback;
    testSendRawTransaction: TRpcRequestCallback;
  };
  cosmosRpc: {
...
export function JsonRpcContextProvider({
  ...
  const ethereumRpc = {
    ...
    testEthCall: _createJsonRpcRequestHandler(
      async (chainId: string, address: string) => {
        const caipAccountAddress = `${chainId}:${address}`;
        const account = accounts.find(
          (account) => account === caipAccountAddress
    ...
    testSendRawTransaction: _createJsonRpcRequestHandler(
      async (chainId: string, address: string) => {
        const caipAccountAddress = `${chainId}:${address}`;
        const account = accounts.find(
          (account) => account === caipAccountAddress
        ...
        const rawTx = utils.serializeTransaction({
          ...tx,
    ...
    testRequestAccounts: _createJsonRpcRequestHandler(
      async (chainId: string, address: string) => {
        const result = await client!.request<string[]>({
          topic: session!.topic,
          chainId,
