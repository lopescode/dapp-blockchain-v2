...
const Home: NextPage = () => {
  ...
  const getEthereumActions = (
    ...
    const actions = {
      ...
      [DEFAULT_EIP155_OPTIONAL_METHODS.ETH_CALL]: {
        method: DEFAULT_EIP155_OPTIONAL_METHODS.ETH_CALL,
        callback: async (chainId: string, address: string) => {
          openRequestModal();
          await ethereumRpc.testEthCall(chainId, address);
        },
      },
      [DEFAULT_EIP155_OPTIONAL_METHODS.ETH_REQUEST_ACCOUNTS]: {
        method: DEFAULT_EIP155_OPTIONAL_METHODS.ETH_REQUEST_ACCOUNTS,
        callback: async (chainId: string, address: string) => {
          openRequestModal();
          await ethereumRpc.testRequestAccounts(chainId, address);
        },
      },
      [DEFAULT_EIP155_OPTIONAL_METHODS.ETH_SEND_RAW_TRANSACTION]: {
        method: DEFAULT_EIP155_OPTIONAL_METHODS.ETH_SEND_RAW_TRANSACTION,
        callback: async (chainId: string, address: string) => {
          openRequestModal();
          await ethereumRpc.testSendRawTransaction(chainId, address);
        },
      },
    };
    ...
    console.log(session?.namespaces?.["eip155"].methods);
